...
router.post('/login', requestBody, async (req, res) => {
  ...
  if (user) {
    ...
    res
      .status(401)
      .send('Unauthorized. Your username or password is incorrect.');
});
...
router.get(
  '/current-user',
  [isAuthenticatedJWT, requestBody],
  (req: any, res: any) => res.send(req.body.decodedJWT)
);
router.get('/is-logged-in', [isLoggedIn, requestBody], (req: any, res: any) =>
  res.send(req.body.isLoggedIn)
);
