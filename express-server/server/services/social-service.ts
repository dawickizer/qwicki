...
class SocialService {
  ...
  async sendFriendRequest(req: any): Promise<User | null> {
    ...
    if (toUser && fromUser) {
      ...
      if (toUser.id === fromUser.id)
        throw Error('You cannot send a friend request to yourself');
      if (toUser.friends.includes(fromUser._id))
        throw Error('You already are friends with this user');
      for (const fromUserOutboundFriendRequest of fromUser.outboundFriendRequests)
        if (
          toUser.inboundFriendRequests.includes(fromUserOutboundFriendRequest)
        )
      ...
      for (const fromUserInboundFriendRequest of fromUser.inboundFriendRequests)
        if (
          toUser.outboundFriendRequests.includes(fromUserInboundFriendRequest)
        )
      ...
      fromUser = await this.userService.putAndPopulateChildren(
        friendRequest.from,
        fromUser
      );
  ...
  async acceptFriendRequest(req: any): Promise<User | null> {
    ...
    const friendRequestId: any = req.body.friendRequestId;
    ...
    if (requestor && requested) {
      ...
      if (
        !requestor.inboundFriendRequests.includes(friendRequestId) &&
        !requested.outboundFriendRequests.includes(friendRequestId)
      )
        throw Error(
          'A friend request does not exist between you and the other user'
        );

      requestor.friends.push(requested._id);
      requested.friends.push(requestor._id);

      requestor.inboundFriendRequests.splice(
        requestor.inboundFriendRequests.indexOf(friendRequestId),
        1
      );
      requested.outboundFriendRequests.splice(
        requested.outboundFriendRequests.indexOf(friendRequestId),
        1
      );

      requestor = await this.userService.putAndPopulateChildren(
        requestor._id,
        requestor
      );
  ...
  async revokeFriendRequest(req: any): Promise<User | null> {
    ...
    if (requestor && requested) {
      // confirm friend request really exists between the two users
      if (
        !requestor.outboundFriendRequests.includes(friendRequestId) &&
      ...
      requestor.outboundFriendRequests.splice(
        requestor.outboundFriendRequests.indexOf(friendRequestId),
        1
      );
      requested.inboundFriendRequests.splice(
        requested.inboundFriendRequests.indexOf(friendRequestId),
        1
      );
