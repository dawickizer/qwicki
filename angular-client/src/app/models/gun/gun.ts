...
export class Gun {
  ...
  async importGunMesh(scene: Scene): Promise<Mesh> {
    this.gunMesh = (
      await SceneLoader.ImportMeshAsync('', this.gunMeshURL, '', scene)
    ).meshes[0] as Mesh;
  ...
  async importReloadSound(scene: Scene): Promise<Sound> {
    ...
    this.reloadSound.setVolume(0.2);
  ...
  async importCockingSound(scene: Scene): Promise<Sound> {
    ...
    this.cockingSound.setVolume(0.2);
  ...
  async createGunMesh(type: string, scene: Scene): Promise<Mesh> {
    if (type == 'primary') {
      ...
      cube.scaling = new Vector3(0.25, 0.25, 3);
      ...
      // get the gun in a world position that is good for baking the verticies
      cube.position = new Vector3(4, -6, 20);
      cube.scaling = new Vector3(0.25, 0.25, 1);
      cube.rotationQuaternion = null;
      cube.rotation = new Vector3(0, 0, 0);
      cube.material = mat;
      cube.isPickable = false;
      ...
      const cube = MeshBuilder.CreateBox('cube', { size: 10 }, scene);
      const mat = new StandardMaterial('spare', scene);
      mat.diffuseColor = Color3.Yellow();

      // get the gun in a world position that is good for baking the verticies
      cube.position = new Vector3(4, -6, 20);
      cube.scaling = new Vector3(0.6, 0.25, 3);
      cube.rotationQuaternion = null;
      cube.rotation = new Vector3(0, 0, 0);
      cube.material = mat;
      cube.isPickable = false;
